...
const validations = {
  ...
  password: (value) => {
    ...
    if (!value) {
      ...
      message = 'Invalid password - It must contains capital letters, lowercase letters and numbers';
    }
    return message;
  },
  course: (value) => {
...
export default class Register extends Component {
  ...
  render() {
    ...
    const campusOptions = campus.map(c => <option key={c} value={c}>{c}</option>);
    const courseOptions = courses.map(c => <option key={c} value={c}>{c}</option>);
